...
- [x] Post-migration, verified the full logging path against the real table: logged a day with a note on `/tech-support`, confirmed "Logged 1 tech support day" and the entry appeared in the list with its note and a Remove link; confirmed it correctly flowed into `/admin/export`'s preview table as `Pay/Tech Support/Units: 1` on Ryan Thomas's row for the current pay period; removed the test entry via its Remove link and confirmed it disappeared from the list (self-delete-within-window RLS policy working).

## payroll_id column fix (0015_fix_payroll_id_column.sql) ✅ verified live

- [x] Discovered live (not from code) that `0012_payroll_id.sql` had never actually been applied - `profiles.payroll_id` didn't exist. What did exist was a column added by hand through the Table Editor UI, literally named `Employee ID` (space, capitals, numeric type), with 10 employees' real GRIN IDs already entered into it by the user - found by querying `select('*')` on `profiles` from the browser console and inspecting the returned keys, since a missing column silently reads as `undefined` rather than erroring when it's not explicitly selected (`select('*')` doesn't fail on a column that isn't there; only explicitly requesting it by name did).
- [x] `0015_fix_payroll_id_column.sql` reconciles this - creates the correctly-named `payroll_id text` column (`if not exists`, safe either way), copies the 10 real values over from `"Employee ID"`, drops the stray column, and unconditionally re-applies the `profiles_update_own_limited` self-edit lockout so `payroll_id` is covered regardless of whether any part of 0012 partially ran.
- [x] Verified live post-migration: `/admin/export` preview now shows the real EmployeeID for all 9 employees who had one set (1033 Brian Pitre, 1024 Chris Wiles, 1040 Dustin Vandyke, 1019 Erik Fox, 1032 Jared Gambrel, 1001 Nathan Boyer, 1035 Ryan Thomas, 1027 Tyson Cox, 1042 Wayne Judd), matching the values shown in the user's own Table Editor screenshot exactly; Josh Springer correctly still blank (his was `NULL`); "missing a Payroll ID" hint count dropped from 10 to 1 accordingly.
- [x] Confirmed via browser console query that `profiles`'s column set no longer includes `"Employee ID"` and does include `payroll_id`.
- [x] No code changes needed - `payrollExport.ts` already read `profiles.payroll_id`, it only needed the column to exist under that name.
...
- [x] Real EmployeeID values now verified (see "payroll_id column fix" below) - this originally only verified the blank-value path since the column didn't actually exist yet at the time.
